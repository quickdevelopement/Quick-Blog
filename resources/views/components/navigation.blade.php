...
<nav class="bg-white border-gray-200 dark:bg-gray-900 shadow-md shadow-gray-400 mb-10">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{ route('posts.index') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Quick Blog</span>
        </a>
            ...
            <button type="button" id="{{ $isLogin ? 'user-menu-button' : '' }}"
                ...
                @if($isLogin)
                    <img class="w-8 h-8 rounded-full"
                        ...
                        alt="user photo">
                @else
                    <a href="{{ route('login.get') }}" class="bg-red-500 px-4 py-2 text-white text-lg rounded">Login</a>
                @endif
